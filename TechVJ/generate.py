...
async def main(bot: Client, message: Message):
    ...
    try:
        client = Client(
            f"{sessions_dir}/temp_{message.from_user.id}",  # Modified path
            api_id=API_ID,
            api_hash=API_HASH,
        ...
        try:
            ...
            if user_data is None:
                uclient = Client(
                    f"{sessions_dir}/user_{message.from_user.id}",  # Modified path
                    api_id=API_ID,
                    api_hash=API_HASH,
            ...
            return await message.reply_text(f"Error connecting client: {str(e)}")
    except Exception as e:
        print(f"Login error: {e}")
        await message.reply_text("An error occurred during login. Please try again.")
